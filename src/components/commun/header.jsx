...
const normalizeRole = (raw) => {
  ...
  // mapper les variantes connues
  if (r === 'USER') return 'UTILISATEUR';
  return r; // UTILISATEUR, PSYCHIATRE, PSYCHOLOGUE, ADMIN
};
...
const Header = () => {
  ...
  // Sync role depuis localStorage (changement de tab, login/logout)
  useEffect(() => {
          ...
          <nav className="flex items-center gap-5">
            ...
            {showMenu && (
              <div className="relative" ref={menuRef}>
                ...
                <div className={`absolute right-0 mt-2 w-52 bg-white border border-gray-200 shadow-lg rounded-xl flex flex-col z-50
                ...
                >
                  <Link
                    to={espaceLink}
                    className="px-4 py-2.5 hover:bg-gray-50 flex items-center gap-2 text-sm text-slate-700 font-medium"
                    onClick={() => setUserMenuOpen(false)}
                  >
                    <User className="w-4 h-4 text-indigo-500" /> {espaceLabel}
                  </Link>

                  {isUser && !isPremium && (
                    <Link
                      to="/devenir-premium"
                      className="px-4 py-2.5 hover:bg-yellow-50 flex items-center gap-2 text-sm text-yellow-600 font-medium"
                      onClick={() => setUserMenuOpen(false)}
                    >
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 hover:bg-red-50 flex items-center gap-2 text-sm text-red-500 font-medium w-full text-left"
                  >
            ...
            {!showMenu && currentRole && (
              <button onClick={handleDeconnexion} className="flex items-center gap-1 text-red-500 hover:text-red-600 text-sm font-medium">
                <LogOut className="w-4 h-4" /> Déconnexion
              </button>
            )}
